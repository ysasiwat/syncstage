...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--root", dest="roots", action="append",
    ...
    org = sub.add_parser("organize", help="Organize into YYYY/MM/<ext>/ under an 'Organized' area.")
    # uses global --apply

    dd = sub.add_parser("dedupe", help="Find duplicates; delete or hardlink duplicates to a keeper.")
    dd.add_argument("--hardlink", action="store_true",
    ...
    rn.add_argument("--template", default="{created:%Y-%m-%d} {stem}{ext}",
    ...
    rn.add_argument("--keep-ext", action="store_true",
                    help="Do not auto-append original extension when {ext} is missing.")
    rn.add_argument("--case", choices=["smart","title","lower","upper","keep"], default="smart",
                    help="Casing for the filename (without extension). Default: smart.")
    rn.add_argument("--ext-case", choices=["keep","lower","upper"], default="keep",
                    help="Casing for the extension. Default: keep.")
    rn.add_argument("--keep-symbols", action="store_true",
                    help="Do NOT strip symbols like _ \" ' ? (keeps them).")
    rn.add_argument("--keep-underscores", action="store_true",
                    help="Do NOT convert underscores to spaces.")
    rn.add_argument("--convert-dashes", action="store_true",
                    help="Convert '-' to spaces too.")
    rn.add_argument("--sanitize-mode", choices=["drop","underscore"], default="drop",
